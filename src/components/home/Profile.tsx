...
export default function Profile({
    ...
    // Keep only: Email, Google Scholar, ORCID, GitHub
    // Explicitly define isEmail for every item to avoid TypeScript union errors.
    const socialLinks = [
        ...(social.email
            ? [
                  {
    ...
    return (
        ...
        >
            {/* Profile Image */}
            <div className="w-52 h-60 sm:w-56 sm:h-64 mx-auto mb-5 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105">
                <Image
                    src={author.avatar}
                    alt={author.name}
                    width={224}
            ...
            {/* Contact Links */}
            <div className="flex justify-center items-center gap-6 sm:gap-8 mb-6 relative px-2">
                        ...
                        return (
                            <div key={link.name} className="relative">
                                <button
                                    ...
                                    onMouseLeave={() => {
                                        if (!isEmailPinned) setShowEmail(false);
                                    }}
                                    onClick={() => {
                                        const nextPinned = !isEmailPinned;
                                        setIsEmailPinned(nextPinned);
                                        setShowEmail(nextPinned);
                                    }}
                                ...
                                <AnimatePresence>
                                    {(showEmail || isEmailPinned) && (
                                        <motion.div
                                            ...
                                            className="absolute top-0 left-1/2 z-20 transform -translate-x-1/2 -translate-y-full bg-neutral-800 text-white px-4 py-3 rounded-lg text-sm font-medium shadow-lg max-w-[calc(100vw-2rem)] sm:max-w-none sm:whitespace-nowrap"
                                            onMouseEnter={() => {
                                                if (!isEmailPinned) setShowEmail(true);
                                            }}
                                            onMouseLeave={() => {
                                                ...
                                                <div className="mt-2">
                                                    ...
                                                    >
                                                        ...
                                                        <span className="sm:hidden">
                                                            {messages.profile.send}
                                                        </span>
                                                        <span className="hidden sm:inline">
                                                            {messages.profile.sendEmail}
                                                        </span>
                                                    </a>
                                                </div>
                                            </div>
